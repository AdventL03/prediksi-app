...
def read_csv_with_multiple_delimiters(filepath):
    try:
        # Coba baca dengan koma sebagai delimiter
        df = pd.read_csv(filepath)
        # Periksa apakah file terbaca dengan benar (misal, cek jumlah kolom atau jika semua data masuk ke satu kolom)
        # Anda bisa menambahkan pemeriksaan yang lebih spesifik di sini
        if df.shape[1] > 1: # Jika jumlah kolom lebih dari 1, kemungkinan delimiter koma benar
            st.write(f"File berhasil dibaca dengan delimiter koma (',').") # Ganti print dengan st.write di Streamlit
            return df
        else: # Jika hanya ada 1 kolom, coba delimiter titik koma
            st.write(f"File kemungkinan menggunakan delimiter titik koma (';'). Mencoba membaca ulang...") # Ganti print dengan st.write
            df = pd.read_csv(filepath, sep=';')
            st.write(f"File berhasil dibaca dengan delimiter titik koma (';').") # Ganti print dengan st.write
            return df
    except Exception as e:
        st.error(f"Error saat membaca file: {e}") # Ganti print dengan st.error
        # Jika terjadi error lain saat membaca dengan koma, langsung coba dengan titik koma
        try:
            df = pd.read_csv(filepath, sep=';')
            st.write(f"File berhasil dibaca dengan delimiter titik koma (';').") # Ganti print dengan st.write
            return df
        except Exception as e_semicolon:
            st.error(f"Error juga saat membaca file dengan delimiter titik koma (';'): {e_semicolon}") # Ganti print dengan st.error
            return None # Kembalikan None jika kedua upaya gagal
        
st.title("Prediksi Waktu Marathon Berdasarkan Data Garmin")

st.markdown("""
### Langkah-langkah Mengunggah Data Garmin:
1. **Buka Website Garmin Connect:**
   - Kunjungi [Garmin Connect](https://connect.garmin.com) dan masuk ke akun Garmin kamu.

2. **Pilih "Activities":**
   - Setelah login, klik menu **Activities** untuk melihat semua aktivitas yang telah kamu lakukan.

3. **Pilih "All Activities":**
   - Di halaman Activities, pilih **Run Activities** untuk melihat daftar aktivitas lengkap kamu.

4. **Scroll ke bawah untuk memilih data selama 6 bulan terakhir:**
   - Misalnya, jika kamu ingin memprediksi berdasarkan maraton yang dilakukan pada **3 Maret 2025**, scroll ke bawah untuk memilih aktivitas dari **September 2024 hingga Maret 2025** (6 bulan sebelumnya).

5. **Klik "Export CSV":**
   - Setelah kamu menemukan aktivitas yang relevan, klik **Export CSV** yang ada di pojok kanan atas halaman untuk mengunduh data dalam format CSV.

6. **Buka dan Simpan File:**
   - Setelah file CSV berhasil diunduh, buka file tersebut, dan simpan menggunakan **Ctrl+S** untuk menyimpannya di folder yang diinginkan.

7. **Upload ke Streamlit:**
   - Kembali ke aplikasi **Streamlit**, klik tombol **Browse Files** dan unggah file **CSV** yang telah kamu unduh tadi.
""")

uploaded_file = st.file_uploader("Unggah file aktivitas Garmin Anda (.csv)", type=["csv"])

df = None # Inisialisasi df
if uploaded_file is not None:
    df = read_csv_with_multiple_delimiters(uploaded_file)
